  ...
  <layout title="Leave Appliction">
    ...
    <vx-card>
      <form @submit.prevent="store">
        <div class="vx-row">
          <div class="vx-col text-center sm:w-1/1 w-full mb-2">
            <vs-avatar size="75px" :src="user.avatar_path" :text="user.name" />
            <br />
  ...
  methods: {
    store() {
      ...
      this.$inertia
        .put(
          this.route(
            "admin.leave-applications.update",
    ...
    reset() {
      ...
      this.$inertia
        .visit(
          this.route("admin.leave-applications.edit", this.leaveApplication.id)
        )
        .then(() => this.$vs.loading.close());
    }
  }
