    ...
    <vs-card>
      ...
      <vs-row vs-type="flex" vs-justify="space-between">
        <vs-col vs-type="flex" vs-justify="flex-end" vs-lg="12" vs-sm="6" vs-xs="12" class="mb-2">
          <inertia-link :href="route('admin.sessions.create')">
            <vs-button color="primary" type="filled">New Session</vs-button>
          </inertia-link>
        </vs-col>
      </vs-row>
      ...
      <vs-collapse id="div-with-loading" accordion type="margin" class="p-0 mt-5 mb-2">
        <vs-row class="header-collapse">
          ...
          <vs-col vs-w="4" vs-type="flex" vs-align="center">Session</vs-col>
          <vs-col vs-w="3" vs-type="flex" vs-align="center">Status</vs-col>
          <vs-col vs-w="4" vs-type="flex" vs-align="center">Timestamp</vs-col>
        </vs-row>
              ...
              <vs-row>
                ...
                >
                  <vs-switch
                    ...
                    vs-icon-on="check"
                    color="success"
                    v-model="session.is_active"
                    @click="activeSession(session.id,session.is_active)"
                  >
                ...
                <vs-col
                  ...
                  vs-align="center"
                  class="text-truncate mb_1"
                >{{ session.updated_at }}</vs-col>
              </vs-row>
            </div>
            <vs-row vs-type="flex" vs-justify="flex-end">
              <vx-tooltip color="primary" title="Edit" text="Edit exam seat plan info.">
                <inertia-link :href="route('admin.sessions.edit',session.id)">
                  <vs-button
                    radius
                    color="primary"
                    type="border"
      ...
      <vs-row>
        <vs-col vs-type="flex" vs-justify="center" class="mt-2 pagi">
          <vs-pagination
            :total="sessions.pagination.total_pages"
...
export default {
  ...
  methods: {
    ...
    activeSession(id, value) {
      this.$vs.loading();
      this.$inertia
        .post(this.route("admin.sessions.active", id), {
          preserveScroll: true,
          preserveState: true
        })
    ...
    acceptDelete() {
      this.$vs.loading();
      this.$inertia
        .delete(this.route("admin.sessions.destroy", this.session.id), {
          preserveScroll: true,
          preserveState: true
        })
        .then(() => this.$vs.loading.close());
