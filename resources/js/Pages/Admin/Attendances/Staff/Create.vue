  ...
  <layout title="Staff Attendance">
    <div class="router-header flex flex-wrap items-center mb-6">
      ...
      <div class="ml-auto md:block hidden cursor-pointer">
        <inertia-link :href="route('admin.staff-attendances.index')">
        ...
        <div class="vx-row">
          <div class="vx-col sm:w-1/1 w-full mb-2">
            ...
            <vs-table :data="users">
              ...
              <template>
                <vs-tr v-for="(user,index) in form.users" :key="index">
                  <vs-td>
                   <span> {{ user.name }}</span>
                    <br />
                    <vs-chip color="primary">{{ user.role.name }}</vs-chip>
                  </vs-td>

                  <vs-td>{{ user.user_id }}</vs-td>

                  <vs-td>
                    <vs-switch v-model="user.status">
                  ...
                  <vs-td>
                    <vs-textarea v-model="user.remark" />
                  </vs-td>
                </vs-tr>
              </template>
            </vs-table>
...
export default {
  ...
  methods: {
    store() {
      this.$vs.loading();
      this.$inertia
        .post(this.route("admin.staff-attendances.store"), this.form, {
          preserveScroll: true,
          preserveState: true
        })
    ...
    reset() {
      this.$vs.loading();
      this.$inertia
        .visit(this.route("admin.staff-attendances.create"))
        .then(() => this.$vs.loading.close());
    }
  }
};
