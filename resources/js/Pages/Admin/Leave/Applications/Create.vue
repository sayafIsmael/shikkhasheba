  ...
  <layout title="Leave Appliction">
    <div class="router-header flex flex-wrap items-center mb-6">
      ...
      <vx-breadcrumb
        :links="
            [
                ...
                url: route('admin.leave-applications.index')
                },
                {
                title: 'Create',
                active: true
      ...
      <div class="ml-auto md:block hidden cursor-pointer">
        <inertia-link :href="route('admin.leave-applications.index')">
        ...
        <div class="vx-row">
          ...
          <div class="vx-col sm:w-1/2 w-full mb-2">
            <vs-select
              ...
              autocomplete
              v-model="form.user"
              :danger="!!$page.errors.user"
              :danger-text="$page.errors.user != null ? $page.errors.user[0] : ''"
            >
              <vs-select-item
                :key="index"
                :value="item.id"
          ...
          <div class="vx-col sm:w-1/2 w-full mb-2">
            ...
            <vs-select
              ...
              label="Select Status"
              v-model="form.status"
              :danger="!!$page.errors.status"
              :danger-text="$page.errors.status != null ? $page.errors.status[0] : ''"
            >
              <vs-select-item
                :key="index"
                :value="item"
...
export default {
  ...
  methods: {
    selectRole() {
      ...
      this.$inertia
        .replace(
          this.route("admin.leave-applications.create", {
    ...
    store() {
      this.$vs.loading();
      this.$inertia
        .post(this.route("admin.leave-applications.store"), this.form, {
          preserveScroll: true,
          preserveState: true
        })
        .then(() => this.$vs.loading.close());
    ...
    reset() {
      this.$vs.loading();
      this.$inertia
        .visit(this.route("admin.leave-applications.create"))
        .then(() => this.$vs.loading.close());
    }
  }
};
